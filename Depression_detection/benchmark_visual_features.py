...
VISUAL_DIR = BASE_DIR / "preprocessed_images"
...
def load_visual(ids: list[str]) -> tuple[np.ndarray, pd.DataFrame]:
    rows = []
    status = []
    for pid in ids:
        path = VISUAL_DIR / f"{pid}_image_features.npy"
        if path.exists():
    ...
    return np.vstack(rows), pd.DataFrame(status)
...
def main() -> None:
    ...
    x_train_raw, train_status = load_visual(train_ids)
    x_dev_raw, dev_status = load_visual(dev_ids)
    x_test_raw, test_status = load_visual(test_ids)

    pd.concat(
        [
            train_status.assign(split="train"),
            dev_status.assign(split="dev"),
            test_status.assign(split="test"),
        ],
        ignore_index=True,
    ).to_csv(MODELS_DIR / "visual_feature_quality.csv", index=False)

    model_factories = {
        "Visual_LogisticRegression": lambda: LogisticRegression(max_iter=1500, class_weight="balanced", random_state=RANDOM_SEED),
        "Visual_RandomForest": lambda: RandomForestClassifier(n_estimators=700, class_weight="balanced_subsample", random_state=RANDOM_SEED),
        "Visual_ExtraTrees": lambda: ExtraTreesClassifier(n_estimators=900, class_weight="balanced", random_state=RANDOM_SEED),
        "Visual_GradientBoosting": lambda: GradientBoostingClassifier(n_estimators=80, learning_rate=0.07, max_depth=2, random_state=RANDOM_SEED),
        "Visual_SVC_RBF": lambda: SVC(C=1.2, gamma="scale", class_weight="balanced", probability=True, random_state=RANDOM_SEED),
    }
        ...
        for model_name, factory in model_factories.items():
            ...
            for objective in ["accuracy", "f1", "recall"]:
                ...
                rows.append({
                    ...
                    "Dev_Key_2": dev_key[1],
                    "Dev_Key_3": dev_key[2],
                    "Missing_Train": int((train_status["status"] == "missing").sum()),
                    "Missing_Dev": int((dev_status["status"] == "missing").sum()),
                    "Missing_Test": int((test_status["status"] == "missing").sum()),
                })
    ...
    ensemble = VotingClassifier(
        estimators=[(name.replace("Visual_", "").lower(), factory()) for name, factory in model_factories.items()],
        voting="soft",
        weights=[1, 1, 1, 2, 2],
    )
    ...
    for objective in ["accuracy", "f1", "recall"]:
        ...
        pred = (prob >= threshold).astype(int)
        rows.append({
            "model": "Visual_SoftVotingEnsemble",
            "scaler": "robust",
            "objective": objective,
            "Accuracy": accuracy_score(y_test, pred),
            "Precision": precision_score(y_test, pred, zero_division=0),
    ...
    joblib.dump(ensemble, MODELS_DIR / "Visual_SoftVotingEnsemble_classifier.pkl")
    joblib.dump(robust_scaler, MODELS_DIR / "Visual_RobustScaler.pkl")

    result = pd.DataFrame(rows).sort_values(["Accuracy", "MCC", "F1"], ascending=False)
    output = MODELS_DIR / "visual_feature_benchmark.csv"
    result.to_csv(output, index=False)
    print(result.head(10).round(4).to_string(index=False))
    print(f"Wrote {output}")
    print(f"Wrote {MODELS_DIR / 'visual_feature_quality.csv'}")
